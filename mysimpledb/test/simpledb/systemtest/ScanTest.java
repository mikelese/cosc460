...
public class ScanTest extends SimpleDbTestBase {
	...
	/**
	 * Tests that SeqScan appropriately adds the table aliase to each field name
	 */
	@Test
	public void testTupleDesc() {
		String alias = "myAlias";
		String fieldName0 = "field0";
		String fieldName1 = "field1";
		TupleDesc td = new TupleDesc(new Type[] { Type.INT_TYPE,
				Type.STRING_TYPE }, new String[] { fieldName0, fieldName1 });
		Database.getCatalog().addTable(new TestUtil.SkeletonFile(-1, td),
				SystemTestUtil.getUUID());
		SeqScan ss = new SeqScan(new TransactionId(), -1, alias);
		assertEquals(0,
				ss.getTupleDesc().fieldNameToIndex(alias + "." + fieldName0));
		assertEquals(1,
				ss.getTupleDesc().fieldNameToIndex(alias + "." + fieldName1));
	}
	...
	private void validateScan(int[] columnSizes, int[] rowSizes)
		...
		for (int columns : columnSizes) {
			for (int rows : rowSizes) {
				...
				HeapFile f = SystemTestUtil.createRandomHeapFile(columns, rows,
						null, tuples);
	 ...
	 */
	@Test
	public void testSmall() throws IOException, DbException,
			TransactionAbortedException {
		int[] columnSizes = new int[] { 1, 2, 3, 4 };
		int[] rowSizes = new int[] { 0, 1, 2, 511, 512, 513, 1023, 1024, 1025,
				4096 + r.nextInt(4096) };
	...
	public void testCache() throws IOException, DbException,
		...
		final int PAGES = 30;
		ArrayList<ArrayList<Integer>> tuples = new ArrayList<ArrayList<Integer>>();
		File f = SystemTestUtil.createRandomHeapFileUnopened(1, 992 * PAGES,
				1000, null, tuples);
